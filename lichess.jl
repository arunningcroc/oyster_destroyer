...
function wait_for_game(http)
    ...
    if di["challenge"]["timeControl"]["limit"] >= 60 && di["challenge"]["timeControl"]["increment"] >= 0
    ...
    for line in r.iter_lines()
        ...
        if Chess.sidetomove(a) == WHITE
            ...
            if Chess.ischeckmate(a)
                ...
                highestid = findmax(map(evaluate_pos,nextpositions))[2]
                println("Search completed" )
                println(Chess.tostring(legalmoves[highestid]))
                retstatus = make_lichess_move(http,id,Chess.tostring(legalmoves[highestid]))
                println(retstatus)
                continue
            end
        else
